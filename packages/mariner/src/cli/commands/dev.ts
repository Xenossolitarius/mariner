import { loadConfigFromFile, createServer as createViteServer, type ConfigEnv } from 'vite'
...
// forcing type so we don't get into a 3 hours type typing session
...
serverOptions(program.command('dev', { isDefault: true }).description('Serve Micro Frontends'))
  .option('-p, --port <port>', 'Specify a port number', parseInt)
  .option('-h, --hostname <hostname>', 'Specify a hostname')
  .option('--https', 'Use a HTTPS server')
  .action(async (options) => {
    console.log(STARTING_DEV_SERVER)

    const projects = await configure({ command: 'serve', mode: options.mode || 'development', ...options })
